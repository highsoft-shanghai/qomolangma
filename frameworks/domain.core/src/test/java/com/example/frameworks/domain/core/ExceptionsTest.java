...
import static com.example.frameworks.domain.core.Exceptions.evaluate;
import static com.example.frameworks.domain.core.Exceptions.execute;
...
public class ExceptionsTest {
    ...
    void should_wrap_exception_for_void_functions() throws IOException {
        ...
        assertThatThrownBy(() -> execute(mock::execute)).isInstanceOf(RuntimeException.class).isSameAs(runtimeException);
    }

    @Test
    void should_wrap_unchecked_exception_for_void_functions() throws IOException {
        doThrow(new IOException()).when(mock).execute();
        assertThatThrownBy(() -> execute(mock::execute)).isInstanceOf(RuntimeException.class);
    }
    ...
    void should_wrap_exception_for_functions_with_result() throws IOException {
        ...
        assertThatThrownBy(() -> evaluate(mock::evaluate)).isInstanceOf(RuntimeException.class).isSameAs(runtimeException);
    }
    ...
    void should_wrap_unchecked_exception_for_functions_with_result() throws IOException {
        ...
        assertThatThrownBy(() -> evaluate(mock::evaluate)).isInstanceOf(RuntimeException.class);
    }
    ...
    void should_be_able_to_evaluate_method_when_method_is_good() throws IOException {
        ...
        String evaluate = evaluate(mock::evaluate);
